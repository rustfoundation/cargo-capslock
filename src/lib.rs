...
impl Opt {
    pub fn main(self) -> anyhow::Result<()> {
        ...
        match self.command {
            ...
            Command::Annotate(cmd) => Ok(cmd.main()?),
        }
    }
}
...
pub enum Command {
    ...
    /// Annotate a call graph generated by the `static` or `dynamic` commands
    /// with function-level vulnerability data sourced from RustSec and OSV.
    ///
    /// In practice, this is only likely to be useful with call graphs generated
    /// by `static`, as `dynamic` only records partial call graphs due to its
